...
# Kubernetes and Helm providers are configured from DOKS module outputs.
# Terraform resolves the implicit dependency: DOKS resources are created first,
# then the providers initialise and ArgoCD resources are applied in the same run.
provider "kubernetes" {
...
module "cluster" {
  ...
  project_description       = "Platform cluster — hosts ArgoCD, permanent infrastructure"
}
...
# Labels it as tier:platform so ApplicationSets target it without any CLI step.
